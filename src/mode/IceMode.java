...
public class IceMode implements GameMode {
    ...
    public IceMode() {
        this.validator = new StandardSudokuValidator();
    }
    ...
    public boolean isValidMove(Puzzle puzzle, int row, int col, int value) {
        if (puzzle.isFrozen(row, col)) return false;
    ...
    public int getHint(Puzzle puzzle, int row, int col, Solution solution) {
        if (puzzle.isFrozen(row, col)) return 0;
        return solution.getCell(row, col); // Use solution for correct hint
    }
