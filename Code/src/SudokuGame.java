...
public class SudokuGame extends JFrame implements ActionListener {
    ...
    // Hint System
    private HintSystem.HintGenerator hintGenerator;

    public SudokuGame() {
        gameManager = GameManager.getInstance();
        hintGenerator = new HintSystem.HintGenerator();
        initializeGUI();
        newGame();
    }
    ...
    private void giveHint() {
        ...
        HintSystem.Hint hint = hintGenerator.getBestHint(puzzle, solution);
        if (hint != null) {
            // Show educational hint dialog
            HintDialog dialog = new HintDialog(this, hint, this);
            dialog.setVisible(true);
        } else {
            statusLabel.setText("No hints available - puzzle might be complete!");
        }
    }
    ...
    // NEW METHODS FOR HINT SYSTEM

    public void applyCellValue(int row, int col, int value) {
        if (row >= 0 && row < 9 && col >= 0 && col < 9) {
            cells[row][col].setText(String.valueOf(value));
            cells[row][col].setForeground(new Color(0, 150, 0)); // Green for hints
            puzzle[row][col] = value;
    ...
    public void highlightCell(int row, int col, boolean highlight) {
        if (row >= 0 && row < 9 && col >= 0 && col < 9) {
            if (highlight) {
                cells[row][col].setBackground(new Color(255, 255, 150)); // Light yellow
            } else {
